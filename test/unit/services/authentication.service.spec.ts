...
const LEGAL_OFFICER_TOKEN = "eyJhbGciOiJFZERTQSJ9.eyJpYXQiOjE2MzEyMTc2MTEsImV4cCI6NDc4NDgxNzYxMSwiaXNzIjoiMTJEM0tvb1dEQ3VHVTdXWTNWYVdqQlMxRTQ0eDRFbm1UZ0szSFJ4V0ZxWUczZHFYRGZQMSIsInN1YiI6IjVINE12QXNvYmZaNmJCQ0R5ajVkc3JXWUxyQThIclJ6YXFhOXA2MVVYdHhNaFNDWSJ9.pBYUyYxq2I_HZiYyeJ-rc8ANxVgckLyd2Y1Snu685mDK4fSwanb6EHsMAP47iCtzSxhaB5bDu7zDmY-XMAyuAw"
...
describe('AuthenticationService authenticatedUserIs()', () => {
    ...
    it('authenticates legal officer based on token', async () => {
        ...
        const logionUser = await authenticationService.authenticatedUserIs(request.object(), USER_ADDRESS);
        await logionUser.requireLegalOfficer();
    })
...
describe('AuthenticationService authenticatedUserIsOneOf()', () => {
    ...
    it('authenticates legal officer based on token', async () => {
        ...
        const request = new Mock<Request>();
        request.setup(instance => instance.header("Authorization")).returns("Bearer " + LEGAL_OFFICER_TOKEN)
        const logionUser = await authenticationService.authenticatedUserIsOneOf(request.object(), "FAKE ADDRESS", USER_ADDRESS);
        await logionUser.requireLegalOfficer();
    })
})
